...
class Builder {
  ...
  static defineProps(propTypes, filter = () => true) {
    ...
    Object.getOwnPropertyNames(propTypes).forEach((prop) => {
      if (!hasOwnProperty(self, prop) && filter(prop)) {
        Builder.defineProp.call(self, prop);
      }
    });
  }

  // Alias to theme.getTheme, so that derived builders has access to themes
  getTheme = () => getTheme();

  // override this method to build the concrete instance
  build = () => undefined;
