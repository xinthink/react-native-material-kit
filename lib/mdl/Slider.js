...
const TRACK_EXTRA_MARGIN_V = utils.toPixels(5);
const TRACK_EXTRA_MARGIN_H = utils.toPixels(5);
...
class Slider extends Component {
  ...
  _updateValueByTouch(evt) {
    ...
    this._internalSetValue(_value, Platform.OS === 'ios');  // report changes in 'real-time' on iOS
    this._moveThumb(x);
  }
  ...
  _confirmUpdateValueByTouch(evt) {
    ...
    if (Platform.OS === 'android') {
      // only report changes when confirm on Android, for performance reasons
