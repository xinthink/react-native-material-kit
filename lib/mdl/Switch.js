...
class Switch extends Component {
  ...
  _layoutThumb(x, y, trackLength, trackSize) {
    const trackRadii = trackSize / 2;
    const thumbRadii = this.props.thumbRadius;
    const thumbX0 = this.state.checked ? trackLength - trackRadii : trackRadii;
    ...
    this.setState({
      ...
      thumbFrame: {
        ...
        r: thumbRadii,
        rippleRadii: trackLength - trackRadii * 2,
      },
    });
  }
  ...
  _computeThumbX(toChecked) {
    ...
    const {trackSize, trackRadii} = this.state;
    const dx = (toChecked ? 1 : -1) * (trackSize - trackRadii * 2);
