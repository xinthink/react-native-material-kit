...
class Ripple extends Component {
  ...
  _onLayoutChange({width, height}) {
    this.setState({
      width,
      height,
      ...this._calcLayers(width / 2, height / 2, width, height),
    });
  }

  _calcLayers(x0, y0, width, height) {
    // mask layer
    ...
    if (Platform.OS === 'android'
        ...
        && this.props.maskEnabled && maskRadiiPercent > 0) {
  ...
  _onPointerDown(evt) {
    this.setState({
      ...this._calcLayers(evt.x, evt.y, this.state.width, this.state.height),
    });
    this.showRipple();
  }
